...
pub fn get_configuration() -> Result<Settings, config::ConfigError> {
    // set common configuration path
    let base_path = std::env::current_dir().expect("Failed to determine the current dirrectory");
    let config_dir = base_path.join("configuration");

    // check for running environment and default to local if not specified
    ...
    let settings = config::Config::builder()
        // the source is coming from a configuration file - configuration.yaml
